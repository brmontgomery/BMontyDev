...
For anyone that doesn't wish to leave the page, here willare the steps:
...
11. Create a batch file with the following
	{% raw %}cd [YOUR_SITE'S_ROOT_FOLDER]
	bundle exec jekyll serve{% endraw %}
...
{% raw %}C:/Ruby33-x64/lib/ruby/3.3.0/logger.rb:384:in `level': undefined method `[]' for nil(NoMethodError){% endraw %}
...
For links I had been using the failing code snippet shown below in code pages
	{% raw %}<link rel="stylesheet" href="{{ site.baseurl }}/assets/css/main.css" />{% endraw %}
In order to fix these bad links, I had to change the baseurl on the config.yaml file and change the code snippet itself to
	{% raw %}<link rel="stylesheet" href="{{ site.url }}/assets/css/main.css" />{% endraw %}
